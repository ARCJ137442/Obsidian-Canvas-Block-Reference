...
2. **Canvas 上下文来自事件 DOM**：必须使用 `getCanvasFromEvent` 解析事件所在的 `Document`、窗口和 Canvas；不得用主窗口 `activeLeaf` 猜测副窗口目标。
...
10. **保护已有用户改动**：修改前检查 `git status`；不重置、不覆盖、不删除用户未提交文件。当前采样脚本可能有用户改动，不能顺手纳入无关提交。
